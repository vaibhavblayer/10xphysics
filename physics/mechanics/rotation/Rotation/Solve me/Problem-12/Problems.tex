...
\draw[-Stealth] (-3,2.25)--(-1.3,2.25) node[left, above]{$P$};
\draw [<->, thick] (-2.5,1.5) -- (-2.5,2.25) node[midway, left]{$R/2$};
\draw[dashed, thick] (-3,1.5) --(0,1.5);
\end{tikzpicture}
\end{center}
...
\begin{calculations}
\begin{align*}
\textit{linear impulse} &= \textit{change in linear momentum} \\[3 mm]
\vec{J} &= \Delta \vec{P} \\[5 mm]
\textit{angular impulse} &= \textit{change in angular momentum} \\[3 mm]
\left| \vec{r} \times \vec{J} \right| &= \left| \Delta \vec{L} \right| \\[3 mm]
\dfrac{PR}{2} &= I\omega \\[3 mm]
\dfrac{PR}{2} &= \dfrac{2}{5} mR^2 \omega \\[3 mm]
\omega &= \dfrac{5P}{4mR}
\end{align*}
\end{calculations}
