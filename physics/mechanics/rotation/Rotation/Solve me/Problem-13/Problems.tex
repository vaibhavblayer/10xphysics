...
\begin{question}
A uniform ring of mass $m$ and radius $R$ is rolling without slipping on a horizontal surface. Find the radius of curvature of the trajectory traced out by the point $A$ in the position shown.
\end{question}

{\physics}

\begin{center}
\begin{tikzpicture}[>=stealth,use optics,very thick,every node/.style={scale=0.85},scale=1]
\draw[mirror->] (-3.5,0)--(3.5,0);
\draw[ultra thick] (0,1.5) edge[->,very thick] node[above]{$R$} ([turn]-60:1.5) circle[radius=1.5];
\fill (0,1.5) circle(2pt);
\draw[-Stealth] (0,1.5) edge[dashed] node[at end,above]{$A$} (0,3)--(3,1.5) node[ above]{$v$};
\fill[red] (0,3) circle(2 pt);
\end{tikzpicture}
\end{center}
...
\step[1]
\begin{align*}
v_A &= v_{CM} + R\omega \\[5 mm]
v_A &= v + R\omega \\[5 mm]
		& \textit{as ring is rolling without slipping} \\ 
		& \quad \left( \omega = \dfrac{v}{R} \right) \\[5 mm]
v_A &= 2v
\end{align*}
\end{calculations}
...
\step[2]
\begin{note}
As ring is in uniform speed therefore point $A$ will have radial acceleration only.
\end{note}
\begin{align*}
a_A &= \dfrac{v^2}{r} \\[5 mm]
\omega^2R &=  \dfrac{{v_A}^2}{r} \\[5 mm]
\left( \dfrac{v^2}{R^2} \right) R &=  \dfrac{{2v}^2}{r} \\[5 mm]
r &= 4R
\end{align*}
\end{calculations}
