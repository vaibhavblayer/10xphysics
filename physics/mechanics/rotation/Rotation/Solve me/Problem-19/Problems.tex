...
\begin{question}
A bead of mass $m$ slides on a hemispherical surface with a velocity $v$ at an angular position $\theta$. If the coefficient of friction between the bead and hemispherical surface is $\upmu$ . Find the magnitude of angular momentum of the bead about $O$ in the position shown.
\end{question}
...
\centerarc [mirror->] (0,2.5)(180:360:2.5);
\draw [->] (0,2.5) coordinate (O) --++(2.5,0) coordinate (a) node[midway,above]{$R$};
\draw [dashed] (O)--([turn]-135:2.5) coordinate (b);
\pic [thick,draw=black,"$\theta$", angle eccentricity=1.6,angle radius=0.5 cm] {angle = b--O--a};
\fill [red] (O) node[above]{$O$} circle(2pt);
\fill [black] ($(O)!0.945!(b)$) coordinate (b') circle(5pt);
\draw [->] (b') -- ($(b')!2cm!90:(O)$) node[below]{$v$};
\end{tikzpicture}
\end{center}
...
\begin{calculations}
Simplest problem ever \Smiley[2]


\begin{align*}
\vec{L} &= \vec{r} \times \vec{p} \\[2 mm]
\left| \vec{L}  \right| &= \left| \vec{r} \times \vec{p}  \right| \\[2 mm]
L &= mvr\sin\theta \\
\end{align*}

Given velocity is tangential as shown in figure means angle between velocity and position is $90^\circ$.\\
\[
L = mvR \ans
\]
\end{calculations}
